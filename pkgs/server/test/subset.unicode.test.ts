...
it("rejects out-of-range codepoints (> U+10FFFD)", async () => {
    ...
    const res = await app.request(url, {
        ...
        body: form({
            files,
            config: JSON.stringify({ output: "ttf", unicodes: "U+10FFFD-110000" }),
        }),
    })
...
it("rejects ranges where start > end (e.g., '7F-0')", async () => {
    ...
    const res = await app.request(url, {
        ...
        body: form({
            files,
            config: JSON.stringify({ output: "ttf", unicodes: "7F-0" }),
        }),
    })
...
it("rejects malformed tokens (non-hex)", async () => {
    ...
    const res = await app.request(url, {
        ...
        body: form({
            files,
            config: JSON.stringify({ output: "ttf", unicodes: "U+ZZ,B5" }),
        }),
    })
